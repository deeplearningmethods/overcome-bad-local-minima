...
train_steps = 10000; batch_size = 1024; l_rate = 0.01;
Dim1 = [1, 5, 1]; Dim2 = [1, 20, 1]; Dim3 = [1, 150, 1]
Initializations = 300; M = Initializations; T = train_steps
...
# Initialize ANN parameters: standard normal like
        W = Var((nns[1]**(-1/7))*abs(Rn((M, h0, h1))), 'weights')
        B = Var((nns[1]**(-1/2))*Rn((M, 1, h1)), 'biases')
...
Opt1 = Opt(l_rate).minimize(L1)
Opt2 = Opt(l_rate).minimize(L2)
Opt3 = Opt(l_rate).minimize(L3)

# ANN parameter; Gradient
par1 = var_list[0:4]; g1 = tf.gradients(L1, par1)
par2 = var_list[4:8]; g2 = tf.gradients(L2, par2)
par3 = var_list[8:12]; g3 = tf.gradients(L3, par3)

for i in range(4):
    par1[i] = tf.reshape(par1[i], [M,d[i]])
    par2[i] = tf.reshape(par2[i], [M,d[4+i]])
    par3[i] = tf.reshape(par3[i], [M,d[8+i]])
    g1[i] = tf.reshape(g1[i], [M,d[i]])
    g2[i] = tf.reshape(g2[i], [M,d[4+i]])
    g3[i] = tf.reshape(g3[i], [M,d[8+i]])
...
with tf.compat.v1.Session() as sess:
    ...
    for i in range(T):
        sess.run(Opt1); sess.run(Opt2); sess.run(Opt3)
        if (i%10==0):
...
plt.show
plt.savefig("Opt_SNN_SGD_5_20_150_ReLU_all_param_train.pdf", bbox_inches="tight")