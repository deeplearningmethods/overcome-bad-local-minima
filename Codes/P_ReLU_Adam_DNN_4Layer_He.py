...
# Initialize ANN parameters: He normal
        W = Var(Rn((M, h0, h1), stddev=tf.math.sqrt(2/h0)), 'weights')
...
# Activation; Adam Optimizer; Target function; Risk function
ReLU = tf.nn.relu; Opt = tf.compat.v1.train.AdamOptimizer
def f(x):
    return x**2 + 2*x
...
Opt1 = Opt(learning_rate=learning_rate, beta1=0.9, beta2=0.999,
           epsilon=1e-08, use_locking=False, name='Adam').minimize(L)
...
plt.show
plt.savefig("ReLU_DNN_Adam_1_50_50_1_He.pdf", bbox_inches="tight")